...
export async function PUT(
  ...
  try {
    ...
    if (auth.role === 'ADMIN') {
      ...
      // Non-admin: can only update username/password on their own proxies
      ...
      if (!username && !password) {
        return errorResponse('Provide username and/or password to update');
      }
      ...
      // Restart proxy to apply new credentials
      if (existing.enabled) {
        await proxyManager.stopProxy(params.id);
        await proxyManager.startProxy({
          port: updated.port,
...
export async function DELETE(
  ...
  // Non-admin can only delete their own proxies
  if (auth.role !== 'ADMIN' && existing.assignedToId !== auth.userId) {
    return errorResponse('Forbidden', 403);
  }
  ...
  try {
    ...
    return errorResponse('Failed to delete proxy', 500);
  }
}
